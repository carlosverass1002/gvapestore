...
def generar_factura(venta_id, cliente_nombre, fecha, carrito, subtotal, db_path, usuario_nombre="Desconocido", incluir_cliente=True):
    ...
    try:
        ...
        try:
            ...
            print(f"Impresora predeterminada: {default_printer}")
        except Exception as e:
            messagebox.showerror(
                "Error de Impresión",
                f"No se puede imprimir: {e}. Verifique que una impresora esté configurada y encendida."
            )
            print(f"Error verificando impresora: {e}")
            return pdf_path

        # Enviar a la cola de impresión dos veces usando os.startfile como fallback
        try:
            for i in range(2):
                try:
                    win32api.ShellExecute(0, "print", pdf_path, None, ".", 0)
                    print(f"Enviado a la cola de impresión con ShellExecute ({i + 1}/2)")
                except Exception as e:
                    print(f"ShellExecute falló: {e}. Intentando con os.startfile...")
                    # Fallback: Usar os.startfile para imprimir
                    os.startfile(pdf_path, "print")
                    print(f"Enviado a la cola de impresión con os.startfile ({i + 1}/2)")
        except Exception as e:
            messagebox.showerror(
                "Error de Impresión",
                f"Error al enviar a la cola de impresión: {e}. Verifique que la impresora esté funcionando y que un visor de PDF esté instalado."
            )
